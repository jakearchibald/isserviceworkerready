...
var jsmpeg = self.JSMPEG = function() {
  ...
  this.ready = Promise.resolve().then(async function() {
    await jsmpeg.buffer.waitForBytes(Infinity); // read to end
    await jsmpeg.findStartCode(START_SEQUENCE);
    jsmpeg.firstSequenceHeader = jsmpeg.buffer.index;
    await jsmpeg.decodeSequenceHeader();
  });

  this.readable = new ReadableStream({
    start: () => this.ready,
    pull: controller => {
      return jsmpeg.nextFrame().then(frame => {
...
jsmpeg.prototype.readCode = async function(codeTable) {
  const jsmpeg = this;
  var state = 0;
  do {
    state = codeTable[state + (await jsmpeg.buffer.getBits(1))];
  } while( state >= 0 && codeTable[state] != 0 );
  return codeTable[state+2];
};

jsmpeg.prototype.findStartCode = async function( code ) {
  const jsmpeg = this;
  var current = 0;
  while( true ) {
    current = await jsmpeg.buffer.findNextMPEGStartCode();
...
jsmpeg.prototype.nextFrame = async function() {
  const jsmpeg = this;

  if( !jsmpeg.buffer ) { return; }

  while(true) {
    var code = await jsmpeg.buffer.findNextMPEGStartCode();
    
    if( code == START_SEQUENCE ) {
      await jsmpeg.decodeSequenceHeader();
    }
    else if( code == START_PICTURE ) {
      await jsmpeg.decodePicture();
      return jsmpeg.currentRGBA;
    }
...
jsmpeg.prototype.decodeSequenceHeader = async function() {
  const jsmpeg = this;
  jsmpeg.width = await jsmpeg.buffer.getBits(12);
  jsmpeg.height = await jsmpeg.buffer.getBits(12);
  jsmpeg.buffer.advance(4); // skip pixel aspect ratio
  jsmpeg.pictureRate = PICTURE_RATE[await jsmpeg.buffer.getBits(4)];
  jsmpeg.buffer.advance(18 + 1 + 10 + 1); // skip bitRate, marker, bufferSize and constrained bit

  jsmpeg.initBuffers();

  if( await jsmpeg.buffer.getBits(1) ) { // load custom intra quant matrix?
    for( var i = 0; i < 64; i++ ) {
      jsmpeg.customIntraQuantMatrix[ZIG_ZAG[i]] = await jsmpeg.buffer.getBits(8);
    }
    jsmpeg.intraQuantMatrix = jsmpeg.customIntraQuantMatrix;
  }
  
  if( await jsmpeg.buffer.getBits(1) ) { // load custom non intra quant matrix?
    for( var i = 0; i < 64; i++ ) {
      jsmpeg.customNonIntraQuantMatrix[ZIG_ZAG[i]] = await jsmpeg.buffer.getBits(8);
    }
...
jsmpeg.prototype.decodePicture = async function(skipOutput) {
  const jsmpeg = this;
  jsmpeg.buffer.advance(10); // skip temporalReference
  jsmpeg.pictureCodingType = await jsmpeg.buffer.getBits(3);
  ...
  if( jsmpeg.pictureCodingType == PICTURE_TYPE_P ) {
    jsmpeg.fullPelForward = await jsmpeg.buffer.getBits(1);
    jsmpeg.forwardFCode = await jsmpeg.buffer.getBits(3);
  ...
  if( jsmpeg.pictureCodingType == PICTURE_TYPE_I || jsmpeg.pictureCodingType == PICTURE_TYPE_P ) {
    var 
      tmpY = jsmpeg.forwardY,
      tmpY32 = jsmpeg.forwardY32,
      tmpCr = jsmpeg.forwardCr,
      tmpCr32 = jsmpeg.forwardCr32,
      tmpCb = jsmpeg.forwardCb,
      tmpCb32 = jsmpeg.forwardCb32;
...
jsmpeg.prototype.decodeSlice = async function(slice) {
  const jsmpeg = this;
  ...
  jsmpeg.quantizerScale = await jsmpeg.buffer.getBits(5);
  
  // skip extra bits
  while( await jsmpeg.buffer.getBits(1) ) {
    jsmpeg.buffer.advance(8);
  }

  do {
    await jsmpeg.decodeMacroblock();
    // We may have to ignore Video Stream Start Codes here (0xE0)!?
  } while( !(await jsmpeg.buffer.nextBytesAreStartCode()) );
};
...
jsmpeg.prototype.decodeMacroblock = async function() {
  const jsmpeg = this;

  // Decode macroblock_address_increment
  var 
    increment = 0,
    t = await jsmpeg.readCode(MACROBLOCK_ADDRESS_INCREMENT);
  ...
  jsmpeg.macroblockType = await jsmpeg.readCode(MACROBLOCK_TYPE_TABLES[jsmpeg.pictureCodingType]);
  ...
  if( jsmpeg.macroblockIntra ) {
    ...
    await jsmpeg.decodeMotionVectors();
    jsmpeg.copyMacroblock(jsmpeg.motionFwH, jsmpeg.motionFwV, jsmpeg.forwardY, jsmpeg.forwardCr, jsmpeg.forwardCb);
  }

  // Decode blocks
  var cbp = ((jsmpeg.macroblockType & 0x02) != 0) 
    ? (await jsmpeg.readCode(CODE_BLOCK_PATTERN))
    : (jsmpeg.macroblockIntra ? 0x3f : 0);

  for( var block = 0, mask = 0x20; block < 6; block++ ) {
    if( (cbp & mask) != 0 ) {
      await jsmpeg.decodeBlock(block);
    }
    mask >>= 1;
  }
};
...
jsmpeg.prototype.decodeMotionVectors = async function() {
  const jsmpeg = this;
  ...
  if( jsmpeg.macroblockMotFw ) {
    ...
    if( (code != 0) && (jsmpeg.forwardF != 1) ) {
      r = await jsmpeg.buffer.getBits(jsmpeg.forwardRSize);
  ...
  else if( jsmpeg.pictureCodingType == PICTURE_TYPE_P ) {
...
jsmpeg.prototype.decodeBlock = async function(block) {
  const jsmpeg = this;
  ...
  if( jsmpeg.macroblockIntra ) {
    ...
    if( block < 4 ) {
      ...
      dctSize = await jsmpeg.readCode(DCT_DC_SIZE_LUMINANCE);
    }
    else {
      predictor = (block == 4 ? jsmpeg.dcPredictorCr : jsmpeg.dcPredictorCb);
      dctSize = await jsmpeg.readCode(DCT_DC_SIZE_CHROMINANCE);
    }
    
    // Read DC coeff
    if( dctSize > 0 ) {
      var differential = (await jsmpeg.buffer.getBits(dctSize));
  ...
  while( true ) {
    var 
      run = 0,
      coeff = await jsmpeg.readCode(DCT_COEFF);
    
    if( (coeff == 0x0001) && (n > 0) && ((await jsmpeg.buffer.getBits(1)) == 0) ) {
      // end_of_block
      break;
    }
    if( coeff == 0xffff ) {
      // escape
      run = (await jsmpeg.buffer.getBits(6));
      level = (await jsmpeg.buffer.getBits(8));
      if( level == 0 ) {
        level = (await jsmpeg.buffer.getBits(8));
      }
      else if( level == 128 ) {
        level = (await jsmpeg.buffer.getBits(8)) - 256;
      }
      else if( level > 128 ) {
        level = level - 256;
      }
      ...
      run = coeff >> 8;
      level = coeff & 0xff;
      if( (await jsmpeg.buffer.getBits(1)) ) {
        level = -level;
      }
    }
...
BitReader.prototype.waitForBytes = async function(expectedLen) {
  const bitReader = this;
  while (expectedLen > bitReader._bytesWritten) {
    const result = await bitReader.reader.read();
...
BitReader.prototype.findNextMPEGStartCode = async function() {
  const bitReader = this;
  for( var i = (bitReader.index+7 >> 3); i < bitReader.contentLength; i++ ) {
    await bitReader.waitForBytes(i + 3);

    if(
      bitReader.bytes[i] == 0x00 &&
      bitReader.bytes[i+1] == 0x00 &&
      bitReader.bytes[i+2] == 0x01
    ) {
      bitReader.index = (i+4) << 3;
      return bitReader.bytes[i+3];
    }
  }
...
BitReader.prototype.nextBytesAreStartCode = async function() {
  const bitReader = this;
  var i = (bitReader.index+7 >> 3);

  if (i >= bitReader.contentLength) return false;
  await bitReader.waitForBytes(i + 2);
...
BitReader.prototype.nextBits = async function(count) {
  const bitReader = this;
  var 
    byteOffset = bitReader.index >> 3,
    room = (8 - bitReader.index % 8);

  await bitReader.waitForBytes((bitReader.index + count -1) >> 3);

  if( room >= count ) {
    return (bitReader.bytes[byteOffset] >> (room - count)) & (0xff >> (8-count));
  }

  var 
    leftover = (bitReader.index + count) % 8, // Leftover bits in last byte
    end = (bitReader.index + count -1) >> 3,
    value = bitReader.bytes[byteOffset] & (0xff >> (8-room)); // Fill out first byte
...
BitReader.prototype.getBits = async function(count) {
  const bitReader = this;
  var currentIndex = bitReader.index;
  var value = await bitReader.nextBits(count);
