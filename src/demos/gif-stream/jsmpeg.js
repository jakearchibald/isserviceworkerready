...
var jsmpeg = self.JSMPEG = function() {
  ...
  this.ready = spawn(function*() {
    ...
    jsmpeg.findStartCode(START_SEQUENCE);
    jsmpeg.firstSequenceHeader = jsmpeg.buffer.index;
    jsmpeg.decodeSequenceHeader();
  });

  this.readable = new ReadableStream({
    start: controller => this.ready,
    pull: controller => {
      var frame = jsmpeg.nextFrame();
...
jsmpeg.prototype.writeEnd = function(bytes) {
...
BitReader.prototype.waitForBytes = function(expectedLen) {
  ...
  return spawn(function*() {
    while (expectedLen > bitReader.bytes.length) {
      const result = yield bitReader.reader.read();
      if (result.done) return;
      //Array.prototype.push.apply(bitReader.bytes, result.value); // Callstack issues
      for (var bitReaderStart = bitReader.bytes.length, i = 0; i < result.value.length; i++) {
        bitReader.bytes[bitReaderStart + i] = result.value[i];
      }
    }
  });
};
...
BitReader.prototype.findNextMPEGStartCode = function() {  
  for( var i = (this.index+7 >> 3); i < this.bytes.length; i++ ) {
    if(
      this.bytes[i] == 0x00 &&
      this.bytes[i+1] == 0x00 &&
      this.bytes[i+2] == 0x01
  ...
  this.index = (this.bytes.length << 3);
  return BitReader.NOT_FOUND;
};
