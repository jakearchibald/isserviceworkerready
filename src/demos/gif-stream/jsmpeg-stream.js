...
var jsmpeg = self.JSMPEG = function() {
  ...
  this.ready = spawn(function*() {
    yield jsmpeg.buffer.waitForBytes(Infinity); // read to end
    yield jsmpeg.findStartCode(START_SEQUENCE);
    jsmpeg.firstSequenceHeader = jsmpeg.buffer.index;
    yield jsmpeg.decodeSequenceHeader();
  });
...
jsmpeg.prototype.readCode = function(codeTable) {
  ...
  return spawn(function*() {
    var state = 0;
    do {
      state = codeTable[state + (yield jsmpeg.buffer.getBits(1))];
...
jsmpeg.prototype.findStartCode = function( code ) {
  ...
  return spawn(function*() {
    var current = 0;
    while( true ) {
      current = yield jsmpeg.buffer.findNextMPEGStartCode();
...
jsmpeg.prototype.nextFrame = function() {
  ...
  return spawn(function*() {
    ...
    while(true) {
      var code = yield jsmpeg.buffer.findNextMPEGStartCode();
      
      if( code == START_SEQUENCE ) {
        yield jsmpeg.decodeSequenceHeader();
      }
      else if( code == START_PICTURE ) {
        yield jsmpeg.decodePicture();
...
jsmpeg.prototype.decodeSequenceHeader = function() {
  ...
  return spawn(function*() {
    jsmpeg.width = yield jsmpeg.buffer.getBits(12);
    jsmpeg.height = yield jsmpeg.buffer.getBits(12);
    jsmpeg.buffer.advance(4); // skip pixel aspect ratio
    jsmpeg.pictureRate = PICTURE_RATE[yield jsmpeg.buffer.getBits(4)];
    jsmpeg.buffer.advance(18 + 1 + 10 + 1); // skip bitRate, marker, bufferSize and constrained bit

    jsmpeg.initBuffers();

    if( yield jsmpeg.buffer.getBits(1) ) { // load custom intra quant matrix?
      for( var i = 0; i < 64; i++ ) {
        jsmpeg.customIntraQuantMatrix[ZIG_ZAG[i]] = yield jsmpeg.buffer.getBits(8);
      }
      jsmpeg.intraQuantMatrix = jsmpeg.customIntraQuantMatrix;
    }
    
    if( yield jsmpeg.buffer.getBits(1) ) { // load custom non intra quant matrix?
      for( var i = 0; i < 64; i++ ) {
        jsmpeg.customNonIntraQuantMatrix[ZIG_ZAG[i]] = yield jsmpeg.buffer.getBits(8);
      }
...
jsmpeg.prototype.decodePicture = function(skipOutput) {
  ...
  return spawn(function*() {
    ...
    jsmpeg.pictureCodingType = yield jsmpeg.buffer.getBits(3);
    ...
    if( jsmpeg.pictureCodingType == PICTURE_TYPE_P ) {
      jsmpeg.fullPelForward = yield jsmpeg.buffer.getBits(1);
      jsmpeg.forwardFCode = yield jsmpeg.buffer.getBits(3);
...
jsmpeg.prototype.decodeSlice = function(slice) {
  ...
  return spawn(function*() {
    ...
    jsmpeg.quantizerScale = yield jsmpeg.buffer.getBits(5);
    
    // skip extra bits
    while( yield jsmpeg.buffer.getBits(1) ) {
      jsmpeg.buffer.advance(8);
    }

    do {
      yield jsmpeg.decodeMacroblock();
      // We may have to ignore Video Stream Start Codes here (0xE0)!?
    } while( !(yield jsmpeg.buffer.nextBytesAreStartCode()) );
  });
};
...
jsmpeg.prototype.decodeMacroblock = function() {
  ...
  return spawn(function*() {
    ...
    var 
      increment = 0,
      t = yield jsmpeg.readCode(MACROBLOCK_ADDRESS_INCREMENT);
    ...
    jsmpeg.macroblockType = yield jsmpeg.readCode(MACROBLOCK_TYPE_TABLES[jsmpeg.pictureCodingType]);
    ...
    if( jsmpeg.macroblockIntra ) {
      ...
      yield jsmpeg.decodeMotionVectors();
      jsmpeg.copyMacroblock(jsmpeg.motionFwH, jsmpeg.motionFwV, jsmpeg.forwardY, jsmpeg.forwardCr, jsmpeg.forwardCb);
    }

    // Decode blocks
    var cbp = ((jsmpeg.macroblockType & 0x02) != 0) 
      ? (yield jsmpeg.readCode(CODE_BLOCK_PATTERN))
      : (jsmpeg.macroblockIntra ? 0x3f : 0);

    for( var block = 0, mask = 0x20; block < 6; block++ ) {
      if( (cbp & mask) != 0 ) {
        yield jsmpeg.decodeBlock(block);
      }
      mask >>= 1;
    }
  });
...
jsmpeg.prototype.decodeMotionVectors = function() {
  ...
  return spawn(function*() {
    ...
    if( jsmpeg.macroblockMotFw ) {
      // Horizontal forward
      code = yield jsmpeg.readCode(MOTION);
      if( (code != 0) && (jsmpeg.forwardF != 1) ) {
        r = yield jsmpeg.buffer.getBits(jsmpeg.forwardRSize);
  ...
  return spawn(function*() {
    ...
    if( jsmpeg.macroblockIntra ) {
      ...
      if( block < 4 ) {
        ...
        dctSize = yield jsmpeg.readCode(DCT_DC_SIZE_LUMINANCE);
      }
      else {
        predictor = (block == 4 ? jsmpeg.dcPredictorCr : jsmpeg.dcPredictorCb);
        dctSize = yield jsmpeg.readCode(DCT_DC_SIZE_CHROMINANCE);
      }
      
      // Read DC coeff
      if( dctSize > 0 ) {
        var differential = (yield jsmpeg.buffer.getBits(dctSize));
    ...
    while( true ) {
      var 
        run = 0,
        coeff = yield jsmpeg.readCode(DCT_COEFF);
      
      if( (coeff == 0x0001) && (n > 0) && ((yield jsmpeg.buffer.getBits(1)) == 0) ) {
        // end_of_block
        break;
      }
      if( coeff == 0xffff ) {
        // escape
        run = (yield jsmpeg.buffer.getBits(6));
        level = (yield jsmpeg.buffer.getBits(8));
        if( level == 0 ) {
          level = (yield jsmpeg.buffer.getBits(8));
        }
        else if( level == 128 ) {
          level = (yield jsmpeg.buffer.getBits(8)) - 256;
        }
        else if( level > 128 ) {
          level = level - 256;
        }
        ...
        run = coeff >> 8;
        level = coeff & 0xff;
        if( (yield jsmpeg.buffer.getBits(1)) ) {
          level = -level;
        }
      }
...
BitReader.prototype.waitForBytes = function(expectedLen) {
  const bitReader = this;

  return spawn(function*() {
    while (expectedLen > bitReader._bytesWritten) {
      const result = yield bitReader.reader.read();
...
BitReader.prototype.findNextMPEGStartCode = function() {
  ...
  return spawn(function*() {
    for( var i = (bitReader.index+7 >> 3); i < bitReader.contentLength; i++ ) {
      yield bitReader.waitForBytes(i + 3);
...
BitReader.prototype.nextBytesAreStartCode = function() {
  ...
  return spawn(function*() {
    ...
    yield bitReader.waitForBytes(i + 2);
...
BitReader.prototype.nextBits = function(count) {
  ...
  return spawn(function*() {
    ...
    yield bitReader.waitForBytes((bitReader.index + count -1) >> 3);
...
BitReader.prototype.getBits = function(count) {
  ...
  return spawn(function*() {
    ...
    var value = yield bitReader.nextBits(count);
